...
type SortCol = 'opened' | 'closed' | 'symbol' | 'side' | 'closeType' | 'pnl' | 'fee' | 'sizeUsd' | 'source';
  ...
  return (
    <div className="card border-0 shadow-sm mt-2 overflow-hidden">
      ...
      <div className="d-flex align-items-center justify-content-between px-4 py-3 border-bottom">
        <p className="fw-semibold mb-0" style={{ fontSize: '0.9rem' }}>Historial de Trades</p>
        <span className="badge bg-light text-secondary fw-normal rounded-pill">
      ...
      <div className="table-responsive">
        <table className="table table-hover table-sm align-middle mb-0">
          ...
          <tbody>
            {paginated.map((t, i) => (
              <tr key={t.opened?.getTime?.() ?? i}>
                <td className="text-secondary" style={{ fontSize: '0.8rem' }}>{fmtDate(t.opened)}</td>
                <td className="text-secondary" style={{ fontSize: '0.8rem' }}>{fmtTime(t.opened)}</td>
                <td className="text-secondary" style={{ fontSize: '0.8rem' }}>{fmtTime(t.closed)}</td>
                <td className="font-monospace" style={{ fontSize: '0.8rem' }}>{t.symbol}</td>
                <td>
                  <span className={`badge fw-semibold ${t.side?.toLowerCase() === 'long' ? 'text-success bg-success-subtle' : 'text-danger bg-danger-subtle'}`}
                    style={{ fontSize: '0.65rem', letterSpacing: '0.04em' }}>
                    {t.side}
                  </span>
                </td>
                <td className="text-secondary" style={{ fontSize: '0.8rem' }}>{t.closeType ?? '—'}</td>
                <td className={`fw-semibold ${t.pnl >= 0 ? 'text-success' : 'text-danger'}`}>
                  {t.pnl >= 0 ? '+' : ''}{fmtNum(t.pnl)}
                </td>
                <td className="text-secondary" style={{ fontSize: '0.8rem' }}>{fmtNum(t.fee)}</td>
                <td>{fmtNum(t.sizeUsd)}</td>
                <td className="text-secondary" style={{ fontSize: '0.8rem' }}>{t.source ?? '—'}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {totalPages > 1 && (
        <div className="d-flex align-items-center justify-content-between px-4 py-3 border-top">
          <small className="text-secondary">
            Mostrando {startRow}–{endRow} de {sorted.length} trades
          </small>
          <nav aria-label="Paginación de trades">
