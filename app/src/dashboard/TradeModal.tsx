...
const fetchCandles = async (trade: Trade, signal: AbortSignal): Promise<Candle[]> => {
  ...
  try {
    return await fetchBinanceCandles(sym, interval, s, e, signal);
  } catch {
    return fetchBybitCandles(sym, interval, s, e, signal);
  }
};
...
const TradeChart = ({ trade }: { trade: Trade }) => {
  ...
  useEffect(() => {
    ...
    setStatus('loading');
    fetchCandles(trade, controller.signal)
      .then((candles) => {
        if (cancelled || !seriesRef.current || !chartRef.current) return;
        if (!candles.length) {
          setStatus('error');
          setErrMsg('No market data found.');
          return;
        }
        ...
        <div className="tc-modal-header">
          <div className="tc-trade-title-group">
            <h2 id="tc-modal-title" className="tc-trade-title">
              {trade.symbol}
              <span
                className={`tc-side-badge ${trade.side === 'long' ? 'tc-badge--positive' : 'tc-badge--negative'}`}
              >
                {trade.side}
              </span>
            </h2>
          ...
          <button type="button" className="tc-modal-close" onClick={onClose} aria-label="Close">
            ✕
          </button>
        </div>
          ...
          <div className="tc-trade-stats-card">
            ...
            <div className="tc-trade-stats-grid">
              <div className="tc-trade-stats-col">
                ...
                <StatRow
                  label="Close type"
                  value={trade.closeType}
                  colorClass={closeColors[trade.closeType]}
                />
