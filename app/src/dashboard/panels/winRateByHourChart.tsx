    ...
    <div style={{ width: '100%' }}>
      ...
      <ResponsiveContainer width="100%" height={190}>
        <BarChart data={data} margin={{ top: 8, right: 8, left: -16, bottom: 0 }} barCategoryGap="18%">
          ...
          <Tooltip cursor={{ fill: 'rgba(255,255,255,0.04)' }} content={<HourTooltip textColor={textColor} surfaceColor={surfaceColor} borderColor={borderColor} />} />
          <Bar dataKey="winRate" radius={[3, 3, 0, 0]}>
            {data.map((row) => <Cell key={row.hour} fill={getColor(row.winRate)} fillOpacity={0.85} />)}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
