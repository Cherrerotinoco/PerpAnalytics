    ...
    <div style={{ width: '100%' }}>
      ...
      <ResponsiveContainer width="100%" height={190}>
        <BarChart data={data} margin={{ top: 8, right: 8, left: -8, bottom: 0 }} barCategoryGap="18%">
          ...
          <Tooltip cursor={{ fill: 'rgba(255,255,255,0.04)' }} content={<HourTooltip textColor={textColor} surfaceColor={surfaceColor} borderColor={borderColor} greenColor={greenColor} redColor={redColor} />} />
          <Bar dataKey="pnl" radius={[3, 3, 0, 0]}>
            {data.map((row) => <Cell key={row.hour} fill={row.pnl >= 0 ? greenColor : redColor} fillOpacity={0.82} />)}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
    </div>
